...
export function SectionsPage() {
  ...
  const [form, setForm] = useState({ name: '', course: '', semester: '' });

  const { data: courses } = useCrudList<any>('courses', { limit: 100 });
  const { data: semesters } = useCrudList<any>('semesters', { limit: 100 });
  const { data, isLoading } = useCrudList<Section>('sections', { page, limit: 20 });
  const create = useCrudCreate<Section>('sections');
  const update = useCrudUpdate<Section>('sections');
  const remove = useCrudDelete('sections');

  const openCreate = () => { setEditing(null); setForm({ name: '', course: '', semester: '' }); setOpen(true); };
  const openEdit = (s: Section) => { setEditing(s); setForm({ name: s.name, course: s.course?._id || '', semester: s.semester?._id || '' }); setOpen(true); };
        ...
        <CardContent className="p-0">
          <table className="w-full">
            ...
            <tbody>
              ...
              : data?.data.length === 0 ? <tr><td colSpan={5} className="p-4 text-center text-muted-foreground">No sections found.</td></tr>
              : data?.data.map((s) => (
